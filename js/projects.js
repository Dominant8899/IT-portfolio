...
const DEFAULT_PROJECTS = [
  ...
  },
  {
    ...
    category: 'fullstack',
    categoryLabel: 'Full-Stack Web App',
    summary: 'Full-stack vehicle rental management platform featuring online booking, active rental tracking, database connectivity, and cloud hosting.',
    description: 'Engineered a complete vehicle rental management web application from frontend to backend. Implemented interactive vehicle browsing, customer reservation management, active rental status tracking, database integration with PostgreSQL/MySQL, and live cloud hosting on Vercel.',
    tags: ['React.js', 'Python', 'Django', 'PostgreSQL', 'MySQL', 'Full-Stack', 'Vercel'],
    status: 'Featured',
  ...
  },
  {
    ...
    summary: 'Relational database architecture, stored procedures, and complex query optimization across SQL Server (SSMS/VS Code), PostgreSQL, and MySQL.',
    description: 'Comprehensive database design and management implementation covering ACID transactions, indexing strategies, complex relational joins, data modeling in PostgreSQL, MySQL, MS Access DBMS, and advanced SQL Server Management Studio (SSMS) workflows with VS Code.',
    tags: ['SQL Server (SSMS)', 'PostgreSQL', 'MySQL', 'MS Access DBMS', 'VS Code', 'Database Modeling'],
    status: 'Completed',
  ...
  },
  {
    ...
    summary: 'High-performance implementations of core data structures, graph traversals, and algorithmic problem-solving in C++ and Python.',
    description: 'Engineered efficient algorithmic solutions covering Binary Search Trees, Heaps, Hash Maps, Graph Algorithms (Dijkstra, BFS/DFS), Sorting, and Dynamic Programming with optimal Big-O time and space complexity in C++ and Python.',
    tags: ['C++', 'Python', 'DSA', 'Algorithms', 'Data Structures', 'Problem Solving'],
    status: 'Completed',
  ...
  },
  {
    ...
    summary: 'Web application built with PHP and the Laravel framework, demonstrating MVC architecture, database migrations, and Eloquent ORM.',
    description: 'Active exploration and development with PHP and Laravel. Covers MVC application structure, RESTful routing, Blade templating, Eloquent ORM relations, database migrations in MySQL, and authentication workflows.',
    tags: ['PHP', 'Laravel', 'MySQL', 'MVC', 'Blade', 'Learning'],
    status: 'In Progress',
    image: 'https://images.unsplash.com/photo-1599507593499-a3f7f7d9a2cc?auto=format&fit=crop&w=800&q=80',
    demoUrl: '',
    githubUrl: '',
    isCustom: false
  },
  {
    ...
    summary: 'Object-oriented programming, asynchronous tasks, and backend API engineering using C# and the .NET ecosystem in VS Code.',
    description: 'Hands-on development in C# and .NET exploring strongly-typed object-oriented systems, LINQ data queries, asynchronous controllers, and modern C# developer tooling in VS Code.',
...
function renderProjects() {
  ...
  const filtered = projectsList.filter(proj => {
    ...
    return matchesCategory && (inTitle || inSummary || inDesc || inTags || inCategory);
  });
...
window.openProjectDetails = function(projectId) {
  ...
  const tagsHtml = proj.tags
    .map(tag => `<span class="tech-tag" style="font-size: 0.85rem; padding: 0.35rem 0.75rem;">${escapeHtml(tag)}</span>`)
    .join('');
  ...
  modalContent.innerHTML = `
    ...
    <h4 style="font-size: 1.15rem; margin-bottom: 0.75rem; color: var(--text-heading);">Project Overview & Architecture</h4>
    <p style="font-size: 1rem; line-height: 1.7; margin-bottom: 1.5rem; color: var(--text-secondary);">${escapeHtml(proj.description || proj.summary)}</p>
...
window.openAddProjectModal = function() {
  ...
  document.getElementById('form-project-image').value = 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&w=800&q=80';
...
window.openEditProjectModal = function(projectId) {
  ...
  document.getElementById('form-project-category').value = proj.category || 'fullstack';
  ...
  document.getElementById('form-project-status').value = proj.status || 'Completed';
...
function handleProjectFormSubmit(e) {
  ...
  const description = document.getElementById('form-project-desc').value.trim() || summary;
  ...
  const image = document.getElementById('form-project-image').value.trim() || 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&w=800&q=80';

  if (!title || !summary) {
    if (window.showToast) window.showToast('Please fill in Title and Summary', 'error');
    return;
  }

  const tags = rawTags ? rawTags.split(',').map(t => t.trim()).filter(Boolean) : ['Full Stack'];
  ...
  if (editingProjectId) {
    ...
    if (window.showToast) window.showToast(`New project "${title}" added!`, 'success');
  }
...
window.resetProjectsToDefault = function() {
  if (confirm('Reset projects list to sample defaults? Any custom added projects will be reset.')) {
    projectsList = [...DEFAULT_PROJECTS];
    saveProjectsToStorage();
    renderProjects();
...
window.exportProjectsJSON = function() {
  ...
  if (window.showToast) window.showToast('Projects exported as JSON file', 'success');
};
  ...
  reader.onload = function(e) {
    try {
      ...
      if (Array.isArray(imported) && imported.length > 0) {
        ...
        saveProjectsToStorage();
        renderProjects();
        if (window.showToast) window.showToast(`Successfully imported ${imported.length} projects!`, 'success');
      } else {
        throw new Error('Invalid JSON format');
      }
    } catch (err) {
      if (window.showToast) window.showToast('Failed to import JSON file. Please check file format.', 'error');
    }
  };
...
window.selectPresetImage = function(url, el) {
  ...
  document.querySelectorAll('.preset-thumb').forEach(thumb => thumb.classList.remove('selected'));
